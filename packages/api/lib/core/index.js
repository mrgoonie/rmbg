...
import { defaultMaxResolution } from './types.js';
import { loadModel } from './network.js';
import { calculateProportionalSize, imageDataToFloat32Array } from './utils.js';
export async function rmbg(inputBuffer, { model, maxResolution = defaultMaxResolution, onProgress }) {
    try {
        let progress = 0;
        // Load model
        ...
        const inputTensor = imageDataToFloat32Array(tensorImageData, model.resolution, model.resolution);
        ...
        const [finalWidth, finalHeight] = calculateProportionalSize(originalWidth, originalHeight, maxResolution, maxResolution);
        // Convert to PNG
