...
import { RMBGModel, RMBGOptions, defaultMaxResolution } from './types'
import { loadModel } from './network'
import { calculateProportionalSize, imageDataToFloat32Array } from './utils'
import { ModelCache } from '../config/cache'

export async function rmbgCore(
  inputBuffer: Buffer,
  model: RMBGModel,
  options: {
  ...
  const { maxResolution = defaultMaxResolution, onProgress, cache, enableCache = true } = options
  ...
  try {
    ...
    if (session) {
      try {
        ...
        // Log but don't throw - we don't want cleanup to fail the operation
        console.warn('Failed to release ONNX session:', releaseError)
      }
    }
  }
}
